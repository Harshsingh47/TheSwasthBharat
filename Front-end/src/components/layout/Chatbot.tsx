  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] flex flex-col items-end">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-[380px] md:w-[420px] h-[600px] max-h-[80vh] bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 flex flex-col overflow-hidden relative"
          >
            {/* Header */}
            <div className="p-6 bg-brand-grad text-white relative overflow-hidden shrink-0">
              ...
              <div className="flex items-center justify-between relative z-10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-sm">Swasth Assistant</h3>
            ...
            <div className="px-6 py-4 border-t border-gray-100 bg-white shrink-0">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Common Questions</span>
              </div>
              <div className="flex overflow-x-auto gap-2 pb-2 no-scrollbar">
                {faqs.map((faq, i) => (
                  <button
                    ...
                    className="flex-none flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-primary/5 hover:border-primary/20 border border-gray-100 rounded-xl transition-all cursor-pointer group"
                  >
                    <faq.icon className="w-3.5 h-3.5 text-primary" />
                    <span className="text-xs font-bold text-gray-600 group-hover:text-primary transition-colors whitespace-nowrap">
                      {faq.question}
                    </span>
                  </button>
                ))}
            ...
            <div className="p-6 bg-white border-t border-gray-100 shrink-0">
              ...
              >
                <input 
                  ...
                  className="flex-1 bg-gray-50 border border-gray-100 rounded-xl px-5 py-3 text-sm focus:outline-none focus:border-primary transition-all font-medium"
                />
                <button 
                  type="submit"
                  disabled={!inputValue.trim()}
                  className="w-11 h-11 bg-primary text-white rounded-xl flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-lg shadow-primary/20 disabled:opacity-50 disabled:scale-100 cursor-pointer"
                >
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center shadow-2xl transition-all cursor-pointer relative group ${
      ...
      >
        ...
        {!isOpen && (
          <div className="absolute -top-1 -right-1 w-5 h-5 bg-primary rounded-full flex items-center justify-center text-[10px] font-bold border-2 border-white animate-bounce">
            1
          </div>
        )}
      </motion.button>
